...
- Upstream `ctest` runs during the build (1500 tests incl. self-pack). The package test compiles `hello.c`, packs it with `upx --best`, runs the packed binary, checks `upx -t`, unpacks and runs it again (on Linux/macOS also compares with the original byte for byte; PE files come back slightly different, so Windows only checks the output). Hello is linked statically because a dynamic one is too small to pack.
